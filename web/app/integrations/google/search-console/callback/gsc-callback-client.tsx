...
export function projectIDFromGSCState(state: string) {
  try {
    const payload = state.split(".")[0];
    if (!payload) return "";
    const claims = JSON.parse(decodeBase64URL(payload));
    return typeof claims.project_id === "string" ? claims.project_id : "";
  } catch {
    return "";
  }
}

export function StaticGSCCallbackClient({ code, state, error }: { code: string; state: string; error: string }) {
  const api = useApi();
  const projectId = projectIDFromGSCState(state);
