...
test("canonical Doctor Site Fix APIs use project-scoped lifecycle endpoints", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    return {
      ...
      json: async () =>
        url.endsWith("/finding-links") || (url.endsWith("/site-fixes") && !url.includes("/findings/")) ? [fix] : responseFix,
    };
  };
  ...
  try {
    ...
    const links = await client.listDoctorSiteFixLinks("project-1");
    ...
    assert.deepEqual(
      ...
      [
        ...
        ["https://api.example.test/api/projects/project-1/doctor/finding-links", "GET"],
...
test("topic mutation APIs omit manual creation and keep project scoped update operations", async () => {
  const calls = [];
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (url, init = {}) => {
    calls.push({ url, init });
