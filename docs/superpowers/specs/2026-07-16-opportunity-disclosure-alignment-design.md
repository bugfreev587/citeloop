...
- Move only `Run details` 14 pixels to the right, matching the progress strip's existing `p-3.5` inset.
- Preserve the current vertical spacing, typography, hover and focus treatments, collapsed defaults, and expansion behavior.

## Component Design

`OpportunityFindingProgress` already places `Run timeline` inside a container with `p-3.5`. Both disclosure buttons use the same `px-1` internal padding, so adding `ml-3.5` to the existing `Run details` button produces the matching 14-pixel outer inset without adding a wrapper or coupling the two components.
...
The native button, `aria-expanded`, `aria-controls`, chevron behavior, keyboard interaction, and focus ring remain unchanged. The fixed 14-pixel inset applies at every viewport size because the progress strip uses the same padding at every viewport size.

## Testing

Extend the existing Analysis page contract test to assert that the `Run details` toggle carries the `ml-3.5` alignment class while retaining its disclosure semantics. Run the focused contract test first, followed by the complete Web test suite, typecheck, production build, and Go test suite.
