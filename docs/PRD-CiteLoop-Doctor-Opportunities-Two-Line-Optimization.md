...
> - `docs/PRD-CiteLoop-Workflow-Handoff-Link-Cards.md` 中任何 work-creation、source eligibility 或 routing 条款；link-card mechanics、already-valid-record deep-link behavior 与 exit-state display 保持有效
...
- Current classification 当且仅当记录有 accepted AI Opportunity source 与完整 provenance，record age / pre-boundary origin 不参与判断，因此 provenance-complete 的旧 AI work 仍是 current。Legacy compatibility 当且仅当记录在 2026-07-12 cutover 时已存在且当时 provenance-incomplete；post-cutover provenance-incomplete record 是 invalid，必须 fail closed 进入 quarantine，不获得 Content Plan Section 15.4 legacy operations。
...
- 用户显式触发 AI run 不改变 record ownership：run 只创建 internal candidates；ownership arbitration 后，Doctor-owned candidate 才成为 Doctor finding，eligible growth candidate 才成为 Opportunities 中 `needs_decision` 的 AI Opportunity。Internal candidate 本身不进入任何用户可见 queue。
...
12. 只有在 2026-07-12 cutover 时已存在且当时 provenance-incomplete 的 Topics（包括 source-less 与 partially linked records）按 Content Plan Section 15.4 允许 view/edit、cancel/reschedule、draft/generate 和 archive/dismiss；provenance-complete 旧 AI work 仍是 current，post-cutover incomplete record 则 fail closed 进入 quarantine 且不获得 legacy operations。
...
52. Provenance-complete accepted-AI work 无论年龄都走 current lifecycle；只有 cutover-existing 且当时 provenance-incomplete 的 Topics 按 Content Plan Section 15.4 允许现有记录的 view/edit、cancel/reschedule、draft/generate 和 archive/dismiss。Post-cutover incomplete record 必须 fail closed 进入 quarantine，不获得任何 legacy operation。
