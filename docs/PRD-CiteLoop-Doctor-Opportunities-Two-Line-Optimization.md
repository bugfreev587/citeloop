...
现有 legacy outcomes 按原值迁移：`positive`、`negative`、`mixed`、`inconclusive`、`insufficient_data` 不折叠。旧记录没有 checkpoint role 时，最新 completed checkpoint 映射为 primary，其他记录保留为 historical checkpoints。
...
AI 只提供 advisory semantic judgment，不能代替唯一约束和事务 reservation：
...
- 只有不存在 deterministic conflict 且 AI confidence 达标时才能 reserve。
- reservation 与 Doctor finding/Opportunity/Site Fix/Growth Action 创建必须在同一数据库事务中完成。
- unique conflict 时读取现有 reservation 并 merge evidence，不重试创建第二条 work。
- 事务开始时必须按稳定顺序锁定所有 `conflict_bucket_keys`（registry row lock 或 database advisory lock），然后在锁内完成 deterministic/semantic overlap 查询；仅对 exact hash 加 unique constraint 不足以防止两个不同 hash 的语义等价工作并发创建。
...
| Legacy value | New `ai_run_policy` | Provider call authority |
|---|---|---|
| `automatic` | `scheduled_and_event` | scheduler 与 eligible events 可调用 |
...
- Exact duplicate active work rate = `active work sharing exact_signature_hash / all active work`，按 rolling 30 days，目标 0。
...
- Suppression precision = `correctly suppressed candidates / all suppressed candidates`，每月标注样本目标 >= 95%。
- Suppression recall = `correctly suppressed equivalent candidates / all labeled equivalent candidates`，每月标注样本目标 >= 95%。
...
- 引入 transaction work-signature reservation 和 review-memory registry。
...
Then conflict bucket lock 将 semantic comparison 串行化
And 第二个 worker 必须看到第一个 reservation
