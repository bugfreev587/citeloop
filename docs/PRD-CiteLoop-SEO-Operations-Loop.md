...
### 5.2 Guided permission onboarding

真实用户 onboarding 的第一屏只收一个产品 domain。随后进入类似 SuperX 的 guided setup：用户不需要理解底层 provider 字段，但需要在一个连续流程里授予 CiteLoop 自动运营所需的最小权限。流程目标不是“零授权”，而是“一次性拿齐授权，后续只在高风险动作、异常和策略变更时有限参与”。

Onboarding 必须按用户能理解的语言呈现：

1. **Product domain**：输入产品 domain，CiteLoop 自动发现 canonical host、sitemap、robots、blog/docs path、RSS 和基础技术状态。
2. **Search data**：连接 Google Search Console / Google Analytics。推荐使用 Google OAuth，让用户选择或授权对应 property；如果 property 不存在，CiteLoop 引导创建或验证。用户不填写 `gsc_site_url`、GA4 property id、service account 或 credential ref。
3. **Publishing access**：连接内容发布目标，优先 GitHub App / CMS OAuth / Vercel integration，而不是手填 token。CiteLoop 只请求写入指定 content path、metadata、sitemap 或 deploy hook 的最小权限。
4. **Site verification / tag install**：如果 CiteLoop 有 repo/CMS 写权限，自动提交 Search Console verification meta/tag、GA4/GTM tag 和必要 sitemap 配置；如果没有写权限，生成 DNS TXT、HTML file 或 meta tag 指令并等待验证。
5. **Notification channel**：连接 Slack、Discord 或 email，用于 brief、approval request、auth expiry、publish failure、safe mode。
6. **Autopilot policy**：设置默认自动驾驶等级、低风险自动发布边界、预算、quiet hours、kill switch 和需要人工 review 的动作类型。
7. **Dry run**：CiteLoop 做一次只读 crawl + permission probe + sample draft，不发布；通过后进入 ready state。

每一步都写入 checklist state。用户可以跳过非必需步骤，但系统必须降级能力，而不是把缺失权限伪装成成功。

### 5.3 Domain-only 原则

真实用户可以只输入产品 domain 开始。CiteLoop 后台必须把这个 domain 转换成可运营的 SEO property，并根据站点控制权选择数据接入模式：

1. **公开数据模式**：只依赖公开页面、sitemap、robots、metadata、链接图和可选 SERP provider。该模式无需用户额外配置，但没有 GSC/GA4 私有指标，系统只能生成冷启动机会和技术 SEO 建议。
2. **CiteLoop 托管内容模式**：当 CiteLoop 生成并托管 blog/docs/landing content surface 时，CiteLoop 可以自动注入 GA4/GTM/Search Console verification tag，并用平台托管 integration 读取该托管 surface 的数据。用户仍然从 domain 开始，但 onboarding 会解释 CiteLoop 将托管哪些内容面。
3. **客户自有站点模式**：如果要运营客户已经存在、且不由 CiteLoop 控制发布的站点，仅凭 domain 不能合法读取 GSC/GA4 或写入内容。CiteLoop 可以把验证和授权流程托管成后台任务，但必须通过 Google OAuth、Site Verification token、DNS/HTML/meta 验证、GitHub/repo 写入、CMS OAuth、Vercel integration 或用户已有 Search Console 授权之一完成所有权证明。
...
### 6.0.1 Permission readiness

自动运营需要的权限分成四类：

| readiness key | 目的 | 可接受授权方式 | 缺失时降级 |
|---|---|---|---|
| `search_read` | 读取 GSC search performance、index/sitemap 状态 | Google OAuth、platform managed Search Console property、service account delegated to property | 不生成 CTR/position/decay 类 opportunity |
| `analytics_read` | 读取 GA4 engagement/conversion | Google OAuth、platform managed GA4 property | 不使用 engagement/conversion priority |
| `publisher_write` | 写 blog/docs/metadata/internal links/sitemap/deploy hook | GitHub App、CMS OAuth、Vercel integration、CiteLoop hosted surface | 只能生成 draft，不自动发布 |
| `notification_write` | 发送 brief、approval、failure、safe mode | Slack/Discord/email integration | 不启用 Level 2+ |

Level 2 自动执行的最低权限：`search_read + publisher_write + notification_write + autopilot_policy_confirmed`。`analytics_read` 强烈建议，但缺失时可以进入 Level 2，前提是所有 opportunity 明确标注没有 conversion signal。
...
### 7.2.1 `onboarding_sessions`

记录注册后的 guided setup 流程。一个 project 可以有多次 session，最新 active session 决定 UI checklist。
...
- `input_domain`
- `current_step`：`domain`, `search_data`, `publishing`, `site_verification`, `notifications`, `policy`, `dry_run`
- `readiness_summary`
- `blocked_reason`
- `started_at`
- `completed_at`
...
### 7.2.2 `permission_grants`

记录 CiteLoop 已获得的能力，不直接暴露或存储 raw secret。

字段：

- `id`
- `project_id`
- `grant_type`：`search_read`, `analytics_read`, `publisher_write`, `notification_write`, `site_verify`, `deploy_trigger`
- `provider`：`google_search_console`, `google_analytics`, `github`, `cms`, `vercel`, `slack`, `discord`, `email`, `citeloop_hosted`
- `status`：`missing`, `pending`, `connected`, `expired`, `revoked`, `error`
- `scope_summary`
- `resource_hint`：例如 property、repo、branch、CMS space、channel，不含 secret
- `credential_ref`
- `last_verified_at`
- `expires_at`
- `revoked_at`
...
任何自动执行前必须读取 `permission_grants`，不能只看 UI policy。grant 失效时，相关 executor 必须降级或进入 safe mode。
...
注册后真实用户首先进入 launch checklist，而不是传统设置页：

1. Domain discovered
2. Search data connected
3. Analytics connected
4. Publishing connected
5. Site verification / tags installed
6. Notification connected
7. Autopilot policy confirmed
8. Dry run passed

每项展示：

- status：`not_started`, `in_progress`, `connected`, `optional`, `blocked`
- what CiteLoop can do after this step
- why the permission is needed
- minimum scopes requested
- retry / reconnect / revoke action

Checklist 完成后，首页不再要求用户反复处理权限；后续只在以下场景打扰用户：

- 高风险动作需要 approve。
- Google / publishing / notification grant 失效。
- policy、预算、autopilot level 提升需要确认。
- safe mode、连续 publish failure、严重 traffic anomaly。
- rollback plan 需要人工选择。
...
- 真实用户授权必须优先使用 OAuth/App/integration 模式；个人访问 token、raw webhook URL、service account JSON 只能作为 advanced/admin fallback。
- 所有 stored credential 必须 encrypted at rest，并支持 revoke、reconnect、scope audit。
- Onboarding 必须显示每个 provider 的最小 scopes 和用途；不能用“全站管理权限”换取实现方便。
...
15. 注册后 checklist 能引导用户完成 search data、analytics、publishing、site verification、notification、autopilot policy 和 dry run。
16. 只输入 domain 但未完成授权的项目能进入 `public_only` 模式，并生成 crawl/sitemap/technical 冷启动 brief。
17. UI 不要求真实用户填写 `gsc_site_url`、GA4 property id 或 credential ref。
18. 需要 GSC/GA4 私有数据时，系统展示托管发布或所有权验证路径，而不是把缺失配置作为用户错误。
19. 权限状态写入 `permission_grants`，grant revoked/expired 后相关能力自动降级。
20. Internal link action 能生成 source article patch。
21. 目标文章 content hash 变化时，action_generator abort 并转人工 review。
22. Weekly brief 可生成 5 到 10 个 action，并在 app 内查看。
23. Weekly brief 可通过 verified notification channel 投递。
24. Action 发布后进入 measurement 状态。
25. 7/14/28 天 outcome measurement 可写回。
26. Google auth 失效会显示配置错误并通知。
27. GSC 数据延迟和匿名 query 缺口不会误报为 0 表现。
28. URL Inspection 只用于重点 URL，不做无界全站扫描。
29. 普通 blog 不调用 Google Indexing API。
30. 第二阶段交接给 Autopilot 的前置状态可被机器判断：连续 14 天 sync 成功、最近 28 天 GSC 数据可用、URL normalization 无重复冲突、verified notification channel 存在。
31. `go test ./...` 通过。
32. `web npm run build` 通过。
