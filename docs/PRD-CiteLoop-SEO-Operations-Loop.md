...
记录授权状态，不暴露 raw secret。内部单人 MVP 固定使用 service account + Search Console 站点授权；不实现 Google OAuth callback。
...
- 内部 MVP 使用 service account + GSC property 授权；不做 Google OAuth callback。
...
1. 可通过 service account 连接 Google Search Console property，并验证授权状态。
...
14. Internal link action 能生成 source article patch。
15. 目标文章 content hash 变化时，action_generator abort 并转人工 review。
16. Weekly brief 可生成 5 到 10 个 action，并在 app 内查看。
17. Weekly brief 可通过 verified notification channel 投递。
18. Action 发布后进入 measurement 状态。
19. 7/14/28 天 outcome measurement 可写回。
20. Google auth 失效会显示配置错误并通知。
21. GSC 数据延迟和匿名 query 缺口不会误报为 0 表现。
22. URL Inspection 只用于重点 URL，不做无界全站扫描。
23. 普通 blog 不调用 Google Indexing API。
24. 第二阶段交接给 Autopilot 的前置状态可被机器判断：连续 14 天 sync 成功、最近 28 天 GSC 数据可用、URL normalization 无重复冲突、verified notification channel 存在。
25. `go test ./...` 通过。
26. `web npm run build` 通过。
