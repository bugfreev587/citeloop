...
Pre-boundary or provenance-incomplete Topics are not allowed-source exceptions. This legacy class includes source-less records and records that already link to an Opportunity, Content Action, or Topic but lack any required current-work provenance: accepted source Opportunity ID, source Content Action ID, AI run/model/prompt/evidence provenance, one acceptance timestamp and approval source, and internal Topic linkage when one exists. Per the Content Plan Section 15.4 matrix, these existing records retain view/edit, cancel/reschedule, draft/generate, and archive/dismiss in a labeled compatibility surface. They remain excluded from the current queue and source-integrity metrics and cannot be created, cloned, backfilled, migrated, or used to seed new work.
...
7. Content Plan source integrity: percentage of current Content Plan items with the complete logical provenance chain—accepted source Opportunity ID, source Content Action ID, AI run/model/prompt/evidence provenance, one acceptance timestamp and approval source, and internal Topic linkage when one exists; target is 100%. Every pre-boundary or provenance-incomplete record, whether source-less or partially linked, is classified as legacy compatibility and excluded from this denominator.
...
12. Content Plan is not a new-work source.
   Doctor and Opportunities are the only user-visible new-work sources. Content Plan has no manual Opportunity/Topic/Brief intake; pre-boundary or provenance-incomplete Topics—including source-less and partially linked records—retain only the existing-record operations in the Content Plan Section 15.4 matrix and cannot create, clone, backfill, migrate, or seed work.
