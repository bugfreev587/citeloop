...
Analysis 负责呈现公开数据、GSC/GA4 私有数据、SERP/GEO 信号，并把这些信号转化为可解释的 opportunity 和 action recommendation。Content Plan 只接收从已接受 AI-generated Opportunity 派生的工作，不再承担原始分析、数据连接、机会判断或新工作创建职责。
...
### 0.1 2026-07-12 来源边界更新

本更新采用 Doctor / Opportunities 双线合同，并覆盖本 PRD 中任何允许 manual seed Topic、source-less Content Plan item 或 Content Plan 创建新工作的旧条款：

- Doctor 与 Opportunities 是唯二用户可见的新工作来源。
- Doctor 产生可立即验证的 Site Fix；Opportunities 产生需要 measurement window 的 AI-generated Opportunity。
- 每个新 Content Plan item 必须来自已接受的 AI-generated Opportunity，并保留 source Opportunity ID 与 AI provenance。
- Content Plan 不提供 manual Topic、manual Opportunity 或 manual Content Brief intake。
- 用户可以显式触发 AI Opportunity Finding；这只是 AI run trigger，生成的记录仍是 AI-generated Opportunity，且必须先在 Opportunities 被接受。
- 已存在的 source-less legacy Topics 只保留兼容访问，不迁移、不 backfill，也不能作为新 Content Plan item 的来源。

本 PRD 其余章节中的 `Analysis opportunity` 应按这一更新理解为新版 Opportunities 所拥有的 AI-generated Opportunity；若旧 routing 与较新的双线 owner contract 冲突，以双线合同为准。
...
因此 Phase 1-3 应被视为 IA 重构和页面职责拆分。原 2026-06-24 方向曾计划把 `Opportunities` 重命名为 `Analysis`；2026-07-12 来源边界已覆盖该命名决定，当前用户可见工作源保留为 `Opportunities`，分析能力在其中呈现，`Visibility` 仍收敛为 `Results`。
...
10. Analysis 生成 candidates 并执行 Doctor / Opportunities ownership arbitration
11. 可立即验证的 candidate 进入 Doctor；AI-generated growth candidate 进入 Opportunities
12. 用户接受、暂缓或 dismiss Opportunity
13. 被接受的 Opportunity action 进入 Content Plan / Review / Publish；Doctor work 进入 Site Fixes
...
│  └─ accepted Opportunities route to Content Plan or Results watchlist; Doctor-owned work routes to Doctor/Site Fixes with evidence attached
...
| measured internal-link growth strategy | `Create growth task` |
...
| `near_page_one_query` | average position 8-20 with meaningful impressions | Create refresh brief or measured internal-link task | Accepted AI Opportunity -> Content Plan |
...
| `internal_link_gap` | source pages can support target page | Fix broken/zero links immediately, or propose a measured cluster strategy | Doctor -> Site Fixes for immediate fixes; accepted AI Opportunity -> Content Plan for delayed-growth strategy |
...
Analysis must not assume every candidate creates an Opportunity or that every accepted Opportunity creates a new article.
...
| `add_internal_links` | Doctor -> Site Fixes when immediate link integrity is the success contract; accepted AI Opportunity -> Content Plan when delayed growth is the success contract |
...
Every newly created Content Plan item must come from an accepted AI-generated Opportunity.
...
- accepted AI-generated Opportunity for a delayed-growth internal-link strategy
...
Historical source-less Topics are not allowed-source exceptions. They remain accessible only through a labeled legacy compatibility surface, are excluded from the current queue and source-integrity metrics, and are not migrated or backfilled into Content Briefs.
...
- source AI provenance
- accepted status and timestamp
- evidence summary
...
12. Every newly created Content Plan item has an accepted AI-generated source Opportunity and AI provenance.
...
3. Analysis can generate low CTR, near page-one, and content decay Opportunities from real query/page metrics, while immediately verifiable indexing evidence creates Doctor findings.
...
1. The 2026-06-24 decision to use `Analysis` as the sidebar work-source label is superseded.
   The user-visible growth-work source is `Opportunities`; Search Intelligence or Analysis may describe capabilities inside that page.
...
5. Immediately verifiable technical SEO work belongs to Doctor as Site Fixes, not Content Plan.
   Content Plan receives only accepted AI-generated Opportunity work whose success uses a delayed growth contract. A separate Tasks page remains out of scope.
...
12. Content Plan is not a new-work source.
   Doctor and Opportunities are the only user-visible new-work sources. Content Plan has no manual Opportunity/Topic/Brief intake; historical source-less Topics remain compatibility-only without migration.
