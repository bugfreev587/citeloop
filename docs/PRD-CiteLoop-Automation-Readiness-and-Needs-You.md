...
|---|---:|---|---|
| P0 | 10-49 | red | Blocks execution, safety, publishing, or recovery |
| P1 | 50-69 | amber | Main workflow review or confirmation |
| P2 | 70-99 | blue | Improves results or automation capability |

`tone` 只能作为视觉输出, 不能继续作为业务优先级来源。排序使用 business priority, 同 priority 内再用 numeric priority 细排。实施时可以重新分配当前 numeric priority; 现有数字不再定义业务等级。
...
- `qa_blocking === true` 或 `requires_human_decision === true` 的 blocked drafts 归 P0, 因为它们阻止内容继续 approve/publish。普通 draft review 归 P1。
...
| `monthly_budget_configured` | Autopilot budget | 自动执行缺少 policy budget 边界 | Set Autopilot budget | `/projects/:id/settings#automation-policy` | create in `automation` tab | P0 if `autopilot_level >= 2`, otherwise P2 | Blocking now / Improves results |
...
`monthly_budget_configured` 必须编辑 `SEOPolicy.monthly_budget_limit`, 不得落到 Project config 的 `monthly_budget_usd`; 后者是项目/scheduler budget, 不能解除 Autopilot readiness gate。
...
`priority` 是 P0/P1/P2 业务枚举。`rank` 是同 priority 内的细排数字, 可由现有 numeric `priority` 迁移而来。`tone` 作为视觉输出保留, 但不应再承担业务 priority。Priority 先决定色带和默认 tone, 再用 rank 排序。
