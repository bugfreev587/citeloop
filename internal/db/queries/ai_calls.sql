...
update ai_call_records set
  ...
  verifier_outcome = case when stage = 'fix_grounding_verification'
    and (status in ('ok', 'partial') or (status in ('queued', 'running') and sqlc.arg(status)::text = 'skipped'))
    then coalesce(verifier_outcome, sqlc.narg(verifier_outcome)) else verifier_outcome end,
